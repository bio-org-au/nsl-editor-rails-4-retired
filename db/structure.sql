...
CREATE FUNCTION audit.audit_table(target_table regclass)
  RETURNS void
    LANGUAGE sql
    AS $_$
...
COMMENT ON FUNCTION audit.audit_table(target_table regclass)
IS '
...
COMMENT ON FUNCTION audit.audit_table(target_table regclass, audit_rows boolean, audit_query_text boolean,
                                      ignored_cols text [])
IS '
...
COMMENT ON FUNCTION audit.if_modified_func()
IS '
...
CREATE FUNCTION public.author_notification()
  RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.daily_top_nodes(tree_label text, since timestamp without time zone)
...
CREATE FUNCTION public.find_family_name_id(target_element_link text)
  RETURNS bigint
LANGUAGE sql
AS $$
...
CREATE FUNCTION public.find_name_in_tree(pname bigint, ptree bigint)
  RETURNS bigint
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE FUNCTION public.find_rank(name_id bigint, rank_sort_order integer)
  RETURNS TABLE(name_element text, rank text, sort_order integer)
LANGUAGE sql
AS $$
...
CREATE FUNCTION public.find_tree_rank(tve_id text, rank_sort_order integer)
  RETURNS TABLE(name_element text, rank text, sort_order integer)
LANGUAGE sql
AS $$
WITH RECURSIVE walk (parent_id, name_element, rank, sort_order) AS (
...
CREATE FUNCTION public.instance_notification()
  RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE FUNCTION public.is_instance_in_tree(pinstance bigint, ptree bigint)
  RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
-- Name: map_nodes_element_identifiers(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.map_nodes_element_identifiers()
...
CREATE FUNCTION public.name_name_path(target_name_id bigint)
...
CREATE FUNCTION public.name_notification()
  RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.pbool(bool boolean)
...
CREATE FUNCTION public.profile_as_jsonb(source_instance_id bigint)
  RETURNS jsonb
LANGUAGE sql
AS $$
SELECT jsonb_object_agg(key.name, jsonb_build_object(
    ...
    'source_link', 'http://localhost:7070/nsl-mapper' || '/instanceNote/apni/' || note.id
))
FROM instance i
...
CREATE FUNCTION public.reference_notification()
  RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.synonym_as_html(instance_id bigint)
...
CREATE FUNCTION public.synonyms_as_html(instance_id bigint)
  RETURNS text
LANGUAGE sql
AS $$
...
CREATE FUNCTION public.synonyms_as_jsonb(instance_id bigint, host text)
  RETURNS jsonb
LANGUAGE sql
AS $$
...
CREATE FUNCTION public.tree_element_data_from_start_node(root_node bigint)
  RETURNS TABLE(tree_id bigint, node_id bigint, excluded boolean, declared_bt boolean, instance_id bigint, name_id bigint, simple_name text, name_path text, instance_path text, parent_instance_path text, parent_excluded boolean, depth integer)
LANGUAGE sql
AS $$
...
COMMENT ON COLUMN audit.logged_actions.event_id
...
COMMENT ON COLUMN audit.logged_actions.schema_name
...
COMMENT ON COLUMN audit.logged_actions.table_name
...
COMMENT ON COLUMN audit.logged_actions.relid
...
COMMENT ON COLUMN audit.logged_actions.session_user_name
...
COMMENT ON COLUMN audit.logged_actions.action_tstamp_tx
...
COMMENT ON COLUMN audit.logged_actions.action_tstamp_stm
...
COMMENT ON COLUMN audit.logged_actions.action_tstamp_clk
...
COMMENT ON COLUMN audit.logged_actions.transaction_id
...
COMMENT ON COLUMN audit.logged_actions.application_name
...
COMMENT ON COLUMN audit.logged_actions.client_addr
...
COMMENT ON COLUMN audit.logged_actions.client_port
...
COMMENT ON COLUMN audit.logged_actions.client_query
...
COMMENT ON COLUMN audit.logged_actions.action
...
COMMENT ON COLUMN audit.logged_actions.row_data
...
COMMENT ON COLUMN audit.logged_actions.changed_fields
...
COMMENT ON COLUMN audit.logged_actions.statement_only
...
ALTER SEQUENCE audit.logged_actions_event_id_seq
OWNED BY audit.logged_actions.event_id;
  ...
  SELECT
    ...
    COALESCE(primary_ref.citation, (SELECT r.citation
                                    FROM ((public.instance s
                                      JOIN public.instance_type it
                                        ON (((s.instance_type_id = it.id) AND it.secondary_instance)))
    ...
    WHEN (basionym_inst.id IS NOT NULL)
      THEN ('http://id.biodiversity.org.au/instance/apni/' :: text || (basionym_inst.cites_id) :: text)
    ELSE
      CASE
      WHEN (primary_inst.id IS NOT NULL)
        THEN ('http://id.biodiversity.org.au/instance/apni/' :: text || (primary_inst.id) :: text)
      ELSE NULL::text
      END
    END                                                                             AS "originalNameUsageID",
    CASE
    WHEN (nt.autonym = true)
      THEN (parent_name.full_name) :: text
    ELSE (SELECT string_agg(regexp_replace((note.value) :: text, '[

 ]+' :: text, ' ' :: text, 'g' :: text), ' ' :: text) AS string_agg
          FROM (public.instance_note note
            JOIN public.instance_note_key key1
  ...
  FROM ((((((((((((public.name n
    ...
    LEFT JOIN ((public.instance primary_inst
      JOIN public.instance_type primary_it
    ...
    LEFT JOIN ((public.instance basionym_inst
      JOIN public.instance_type "bit"
        ON ((("bit".id = basionym_inst.instance_type_id) AND (("bit".name) :: text = 'basionym' :: text))))
      JOIN public.name basionym ON ((basionym.id = basionym_inst.name_id)))
      ON ((basionym_inst.cited_by_id = primary_inst.id)))
    LEFT JOIN public.tree_element te ON ((n.id = te.name_id)))
...
CREATE MATERIALIZED VIEW public.taxon_view AS
  SELECT
    (syn.value ->> 'concept_link' :: text)                               AS "taxonID",
    ...
    WHEN syn_nt.autonym
      THEN NULL :: text
    ELSE regexp_replace("substring"((syn_name.full_name_html) :: text, '<authors>(.*)</authors>' :: text),
                        '<[^>]*>' :: text, '' :: text, 'g' :: text)
    END                                                                  AS "scientificNameAuthorship",
    ...
    CASE
    WHEN (firsthybridparent.id IS NOT NULL)
      THEN ('http://id.biodiversity.org.au/name/apni/' :: text || firsthybridparent.id)
    ELSE NULL::text
    ...
    CASE
    WHEN (secondhybridparent.id IS NOT NULL)
      THEN ('http://id.biodiversity.org.au/name/apni/' :: text || secondhybridparent.id)
    ELSE NULL::text
  ...
  SELECT
    ...
    WHEN acc_nt.autonym
      THEN NULL :: text
    ELSE regexp_replace("substring"((acc_name.full_name_html) :: text, '<authors>(.*)</authors>' :: text),
                        '<[^>]*>' :: text, '' :: text, 'g' :: text)
        END                                                                 AS "scientificNameAuthorship",
...
CREATE TABLE public.tree_arrangement (
  ...
  CONSTRAINT chk_classification_has_label CHECK (((tree_type <> ALL (ARRAY ['E' :: bpchar, 'P' :: bpchar])) OR
                                                  (label IS NOT NULL))),
  CONSTRAINT chk_tree_arrangement_type CHECK ((tree_type = ANY
                                               (ARRAY ['E' :: bpchar, 'P' :: bpchar, 'U' :: bpchar, 'Z' :: bpchar]))),
...
CREATE TABLE public.tree_link (
  ...
  CONSTRAINT chk_tree_link_vmethod CHECK ((versioning_method = ANY
                                           (ARRAY ['F' :: bpchar, 'V' :: bpchar, 'T' :: bpchar])))
);
...
CREATE TABLE public.tree_node (
  ...
  CONSTRAINT chk_internal_type_d CHECK ((((internal_type) :: text <> 'D' :: text) OR
                                         ((name_uri_ns_part_id IS NULL) AND (taxon_uri_ns_part_id IS NULL) AND
                                          (literal IS NULL)))),
  CONSTRAINT chk_internal_type_enum CHECK (((internal_type) :: text = ANY
                                            (ARRAY [('S' :: character varying) :: text, ('Z' :: character varying) :: text, ('T' :: character varying) :: text, ('D' :: character varying) :: text, ('V' :: character varying) :: text]))),
  CONSTRAINT chk_internal_type_s CHECK ((((internal_type) :: text <> 'S' :: text) OR (
    (name_uri_ns_part_id IS NULL) AND (taxon_uri_ns_part_id IS NULL) AND (resource_uri_ns_part_id IS NULL) AND
    (literal IS NULL)))),
  CONSTRAINT chk_internal_type_t CHECK ((((internal_type) :: text <> 'T' :: text) OR (literal IS NULL))),
  CONSTRAINT chk_internal_type_v CHECK ((((internal_type) :: text <> 'V' :: text) OR
                                         ((name_uri_ns_part_id IS NULL) AND (taxon_uri_ns_part_id IS NULL) AND
                                          (((resource_uri_ns_part_id IS NOT NULL) AND (literal IS NULL)) OR
                                           ((resource_uri_ns_part_id IS NULL) AND (literal IS NOT NULL)))))),
  CONSTRAINT chk_tree_node_instance_matches CHECK (((instance_id IS NULL) OR
                                                    (((instance_id) :: character varying) :: text =
                                                     (taxon_uri_id_part) :: text))),
  CONSTRAINT chk_tree_node_name_matches CHECK (((name_id IS NULL) OR (((name_id) :: character varying) :: text =
                                                                      (name_uri_id_part) :: text))),
...
ALTER TABLE ONLY audit.logged_actions
  ALTER COLUMN event_id SET DEFAULT nextval('audit.logged_actions_event_id_seq' :: regclass);
...
--
-- Name: tree_node current_name_only_once; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tree_node
  ADD CONSTRAINT current_name_only_once EXCLUDE USING btree (tree_arrangement_id WITH =, name_id WITH =)
...
--
-- Name: help_topic help_topic_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: nomenclatural_event_type nomenclatural_event_type_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_arrangement tree_arrangement_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_event tree_event_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_link tree_link_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_uri_ns tree_uri_ns_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_value_uri tree_value_uri_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: name_type uk_314uhkq8i7r46050kd1nfrs95; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_uri_ns uk_5smmen5o34hs50jxd247k81ia; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_uri_ns uk_70p0ys3l5v6s9dqrpjr3u3rrf; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: locale uk_qjkskvl9hx0w78truoyq9teju; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: name_status uk_se7crmfnhjmyvirp3p9hiqerx; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: why_is_this_here uk_sv1q1i7xve7xgmkwvmdbeo1mb; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_arrangement uk_y303qbh1ijdg3sncl9vlxus0; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: user_query user_query_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: why_is_this_here why_is_this_here_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: identifier_id_version_object_index; Type: INDEX; Schema: mapper; Owner: -
--

CREATE INDEX identifier_id_version_object_index
  ON mapper.identifier
  USING btree (id_number, object_type, version_number);
...
CREATE INDEX identifier_index
  ON mapper.identifier
  USING btree (id_number, name_space, object_type);

--
-- Name: identifier_object_type_index; Type: INDEX; Schema: mapper; Owner: -
--

CREATE INDEX identifier_object_type_index
  ON mapper.identifier
  USING btree (object_type);
...
CREATE INDEX identifier_version_index
  ON mapper.identifier
  USING btree (id_number, name_space, object_type, version_number);
...
CREATE INDEX mapper_identifier_index
  ON mapper.identifier_identities
  USING btree (identifier_id);
...
CREATE INDEX match_host_index
  ON mapper.match_host
  USING btree (match_hosts_id);
...
CREATE INDEX auth_source_index
  ON public.author
  USING btree (namespace_id, source_id, source_system);
...
CREATE INDEX event_record_index
  ON public.event_record
  USING btree (created_at, dealt_with, type);
...
CREATE INDEX id_mapper_from_index
  ON public.id_mapper
  USING btree (from_id, namespace_id, system);
...
CREATE INDEX idx_node_current_a
  ON public.tree_node
  USING btree (tree_arrangement_id)
...
CREATE INDEX idx_node_current_instance_a
  ON public.tree_node
  USING btree (instance_id, tree_arrangement_id)
  WHERE (replaced_at_id IS NULL);
...
CREATE INDEX idx_node_current_name_a
  ON public.tree_node
  USING btree (name_id, tree_arrangement_id)
  WHERE (replaced_at_id IS NULL);
...
CREATE UNIQUE INDEX idx_tree_link_seq
  ON public.tree_link
  USING btree (supernode_id, link_seq);
...
CREATE INDEX idx_tree_node_name
  ON public.tree_node
  USING btree (name_uri_id_part, name_uri_ns_part_id);
...
CREATE INDEX idx_tree_node_name_in
  ON public.tree_node
  USING btree (name_uri_id_part, name_uri_ns_part_id, tree_arrangement_id);
...
CREATE INDEX idx_tree_node_resource
  ON public.tree_node
  USING btree (resource_uri_id_part, resource_uri_ns_part_id);
...
CREATE INDEX idx_tree_node_resource_in
  ON public.tree_node
  USING btree (resource_uri_id_part, resource_uri_ns_part_id, tree_arrangement_id);
...
CREATE INDEX idx_tree_node_taxon
  ON public.tree_node
  USING btree (taxon_uri_id_part, taxon_uri_ns_part_id);
...
CREATE INDEX idx_tree_node_taxon_in
  ON public.tree_node
  USING btree (taxon_uri_id_part, taxon_uri_ns_part_id, tree_arrangement_id);
...
CREATE INDEX instance_note_key_rdfid
  ON public.instance_note_key
  USING btree (rdf_id);
...
CREATE INDEX instance_path_index
  ON public.instance_paths
  USING btree (instance_path);
...
CREATE INDEX instance_type_rdfid
  ON public.instance_type
  USING btree (rdf_id);
...
CREATE INDEX link_uri_index
  ON public.tree_value_uri
  USING btree (link_uri_id_part, link_uri_ns_part_id, root_id);
...
CREATE INDEX lower_full_name
  ON public.name
  USING btree (lower((full_name) :: text));
...
CREATE INDEX name_category_rdfid
  ON public.name_category
  USING btree (rdf_id);
...
CREATE INDEX name_duplicate_of_id_index
  ON public.name
  USING btree (duplicate_of_id);
...
CREATE INDEX name_group_rdfid
  ON public.name_group
  USING btree (rdf_id);
...
CREATE INDEX name_lower_f_unaccent_full_name_like
  ON public.name
  USING btree (lower(public.f_unaccent((full_name) :: text)) varchar_pattern_ops);
...
CREATE INDEX name_lower_full_name_gin_trgm
  ON public.name
  USING gin (lower((full_name) :: text) public.gin_trgm_ops);
...
CREATE INDEX name_lower_simple_name_gin_trgm
  ON public.name
  USING gin (lower((simple_name) :: text) public.gin_trgm_ops);
...
CREATE INDEX name_lower_unacent_full_name_gin_trgm
  ON public.name
  USING gin (lower(public.f_unaccent((full_name) :: text)) public.gin_trgm_ops);
...
CREATE INDEX name_lower_unacent_simple_name_gin_trgm
  ON public.name
  USING gin (lower(public.f_unaccent((simple_name) :: text)) public.gin_trgm_ops);
...
CREATE INDEX name_parent_id_ndx
  ON public.name
  USING btree (parent_id);
...
CREATE INDEX name_sanctioningauthor_index
  ON public.name
  USING btree (sanctioning_author_id);
...
CREATE INDEX name_second_parent_id_ndx
  ON public.name
  USING btree (second_parent_id);
...
CREATE INDEX name_status_rdfid
  ON public.name_status
  USING btree (rdf_id);
...
CREATE INDEX name_whyisthishere_index
  ON public.name
  USING btree (why_is_this_here_id);
...
CREATE INDEX node_uri_index
  ON public.tree_value_uri
  USING btree (node_uri_id_part, node_uri_ns_part_id, root_id);
...
CREATE INDEX nomenclatural_event_type_rdfid
  ON public.nomenclatural_event_type
  USING btree (rdf_id);
...
CREATE INDEX note_key_index
  ON public.instance_note
  USING btree (instance_note_key_id);
...
CREATE INDEX parent_instance_path_index
  ON public.instance_paths
  USING btree (parent_instance_path);
...
CREATE INDEX ref_author_role_rdfid
  ON public.ref_author_role
  USING btree (rdf_id);
...
CREATE INDEX ref_citation_text_index
  ON public.reference
  USING gin (to_tsvector('english' :: regconfig, public.f_unaccent(COALESCE((citation) :: text, '' :: text))));
...
CREATE INDEX reference_authorrole_index
  ON public.reference
  USING btree (ref_author_role_id);
...
CREATE INDEX tree_element_previous_index
  ON public.tree_element
  USING btree (previous_element_id);
...
CREATE INDEX tree_name_path_index
  ON public.tree_version_element
  USING btree (name_path);
...
CREATE UNIQUE INDEX unique_instance_path_index
  ON public.instance_paths
  USING btree (instance_path, excluded);
...
CREATE TRIGGER audit_trigger_row
  ...
  ON public.author
  FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func('true');
...
CREATE TRIGGER audit_trigger_stm
  AFTER TRUNCATE
  ON public.author
  FOR EACH STATEMENT EXECUTE PROCEDURE audit.if_modified_func('true');
...
CREATE TRIGGER author_update
  AFTER INSERT OR DELETE OR UPDATE
  ON public.author
  FOR EACH ROW EXECUTE PROCEDURE public.author_notification();
...
CREATE TRIGGER instance_update
  AFTER INSERT OR DELETE OR UPDATE
  ON public.instance
  FOR EACH ROW EXECUTE PROCEDURE public.instance_notification();
...
CREATE TRIGGER name_update
  AFTER INSERT OR DELETE OR UPDATE
  ON public.name
  FOR EACH ROW EXECUTE PROCEDURE public.name_notification();
...
CREATE TRIGGER reference_update
  AFTER INSERT OR DELETE OR UPDATE
  ON public.reference
  FOR EACH ROW EXECUTE PROCEDURE public.reference_notification();
...
--
-- Name: tree_node fk_16c4wgya68bwotwn6f50dhw69; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_1g9477sa8plad5cxkxmiuh5b; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_link fk_2dk33tolvn16lfmp25nk2584y; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_4y1qy9beekbv71e9i6hto6hun; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_budb70h51jhcxe7qbtpea0hi2; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_value_uri fk_djkn41tin6shkjuut9nam9xvn; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: name fk_dqhn53mdh0n77xhsw7l5dgd38; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_value_uri fk_ds3bc89iy6q3ts4ts85mqiys; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_eqw4xo7vty6e4tq8hy34c51om; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_arrangement fk_fvfq13j3dqv994o9vg54yj5kk; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_gc6f9ykh7eaflvty9tr6n4cb6; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_link fk_kqshktm171nwvk38ot4d12w6b; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_nlq0qddnhgx65iojhj2xm8tay; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_value_uri fk_nw785lqesvg8ntfaper0tw2vs; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_oge4ibjd3ff3oyshexl6set2u; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_pc0tkp9bgp1cxull530y60v46; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_uri_ns fk_q9k8he941kvl07j2htmqxq35v; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: nomenclatural_event_type fk_ql5g85814a9y57c1ifd0nkq3v; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_sbuntfo4jfai44yjh9o09vu6s; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_node fk_t6kkvm8ubsiw6fqg473j0gjga; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_link fk_tgankaahxgr4p0mw4opafah05; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_arrangement tree_arrangement_base_arrangement_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_arrangement tree_arrangement_namespace_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: tree_event tree_event_namespace_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
