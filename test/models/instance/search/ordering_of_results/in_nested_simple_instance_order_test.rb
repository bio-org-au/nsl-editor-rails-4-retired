...
class InNestedSimpleInstanceOrderTest < ActiveSupport::TestCase
  ...
  test "instances in nested simple instance order" do
    ...
    assert_with_args(results, 25, "zzzz99906 - a genus with two instances")
    assert_with_args(results, 26, "999 - a duplicate genus")
    assert_with_args(results, 27, "999 - a_family")
    assert_with_args(results, 28, "999 - a_forma")
    assert_with_args(results, 29, "999 - a_genus")
    assert_with_args(results, 30, "999 - a_nothovarietas")
    assert_with_args(results, 31, "999 - a_sectio")
    assert_with_args(results, 32, "999 - a_series")
    assert_with_args(results, 33, "999 - a_species")
    assert_with_args(results, 34, "74, t. 100 - a_subclassis")
    assert_with_args(results, 35, "999 - a_subforma")
    assert_with_args(results, 36, " - a_subgenus")
    assert_with_args(results, 37, "999 - a_subordo")
    assert_with_args(results, 38, "999 - a_subsectio")
    assert_with_args(results, 39, "999 - a_subseries")
    assert_with_args(results, 40, "999 - a_subspecies")
    assert_with_args(results, 41, "999 - a_subtribus")
    assert_with_args(results, 42, "999 - a_subvarietas")
    assert_with_args(results, 43, "74, t. 99 - a_superordo")
    assert_with_args(results, 44, "999 - a_superspecies")
    assert_with_args(results, 45, "999 - a_tribus")
    assert_with_args(results, 46, "999 - a_varietas")
    assert_with_args(results, 47, " - an unranked with an instance")
    assert_with_args(results, 48, "999 - an_ordo")
    assert_with_args(results,
                     51,
                     "75, t. 102 - Magnoliopsida Brongn. a_classis")
    assert_with_args(results, 52, "75, t. 101 - Metrosideros costata Gaertn.")
    assert_with_args(results, 53, "76 - Metrosideros costata Gaertn.")
    assert_with_args(results, 55, "19-20 - Plantae Haeckel")
  end
end
