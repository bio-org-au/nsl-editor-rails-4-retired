...
class InstanceTypeStandaloneOptionsTest < ActiveSupport::TestCase
  ...
  def test1
    ...
    assert @names.include?("comb. et stat. nov."),
           "Should include comb. et stat. nov."
    assert @names.include?("comb. nov."), "Should include comb. nov."
    assert @names.include?("homonym"), "Should include homonym"
    assert @names.include?("implicit autonym"),
           "Should include implicit autonym"
  end

  def test2
    assert @names.include?("nom. et stat. nov."),
           "Should include nom. et stat. nov."
    assert @names.include?("nom. nov."), "Should include nom. nov."
    assert @names.include?("primary reference"),
           "Should include primary reference"
    assert @names.include?("primary reference"),
           "Should include primary reference"
  ...
  def test4
    ...
    assert_not @names.include?("orthographic variant"),
               "Should not include orthographic variant"
  ...
  def test6
    assert_not @names.include?("vernacular name"),
               "Should not include vernacular name"
  end
end
