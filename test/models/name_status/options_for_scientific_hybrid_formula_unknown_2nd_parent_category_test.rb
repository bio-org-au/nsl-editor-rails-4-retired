...
class OptionsForScientificHybridFormulaUnknown2ndParentCategoryTest < ActiveSupport::TestCase
  ...
  test "should include [deleted]" do
    assert NameStatus.options_for_category(Name::SCIENTIFIC_HYBRID_FORMULA_UNKNOWN_2ND_PARENT_CATEGORY).collect{|e| e.first}.include?('[deleted]'), 
      'Scientific hybrid formula unknown 2nd parent name status options should include "[deleted]"'
  end
 
  test "should include  [n/a]" do
    assert NameStatus.options_for_category(Name::SCIENTIFIC_HYBRID_FORMULA_UNKNOWN_2ND_PARENT_CATEGORY).collect{|e| e.first}.include?('[n/a]'), 
