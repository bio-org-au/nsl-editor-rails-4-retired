...
class NameHybridParentSuggestionsMustAvoidIdTest < ActiveSupport::TestCase
  test "name hybrid parent suggestions must avoid id" do
    ...
    suggestions = Name::AsTypeahead.hybrid_parent_suggestions("angophora costata", name.id + 1)
    ...
    suggestions = Name::AsTypeahead.hybrid_parent_suggestions("angophora costata", name.id)
    assert(suggestions.is_a?(Array), "suggestions should be an array")
