...
class SimpleSearchForOUmlaut < ActiveSupport::TestCase
  ...
  test "simple search for o umlaut with o umlaut" do
    results, rejected_pairings,is_limited,focus_anchor_id,info = Name::AsSearchEngine.search("belonia Körb.")
    ...
    assert_equal names(:belonia_korb_with_diacrit).full_name, results.first[:full_name]
  end

  test "simple search for o umlaut with just o" do
    results, rejected_pairings,is_limited,focus_anchor_id,info = Name::AsSearchEngine.search("belonia Korb.")
    assert_equal results.class, Name::ActiveRecord_Relation, "Results should be a Name::ActiveRecord_Relation."
    assert_equal 1, results.size, "Exactly 1 result is expected."
    assert_equal names(:belonia_korb_with_diacrit).full_name, results.first[:full_name]
  end
