...
class FromCultivarHybridWithOneParentToCultivarTest < ActiveSupport::TestCase
  ...
  test "convert name from cultivar hybrid with one parent - to - cultivar" do
    ...
    # Note: cultivar hybrids with one parent were possibly entered incorrectly in the old APNI.
    assert name.second_parent_id.blank?, 'Expecting 2nd parent to be blank in this fixture.'
    assert_not name.valid?, 'I do not expect a cultivar hybrid name with one parent to be valid'

    name.change_category_to = Name::CULTIVAR_CATEGORY
    assert_equal name.category, Name::CULTIVAR_CATEGORY, 'Name category should now be CULTIVAR.'

    assert_not name.valid?, 'Name should still not be valid now as a cultivar name'
    ...
    name.save!
    assert name.raw_category == Name::CULTIVAR_CATEGORY, 'Name converted from cultivar hybrid should now be a cultivar name'
  end
