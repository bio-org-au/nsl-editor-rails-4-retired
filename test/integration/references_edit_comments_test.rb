...
class ReferencesEditCommentsTest < ActionDispatch::IntegrationTest
  ...
  test "create reference comment" do
    ...
    assert_difference("Comment.count") do
      ...
      assert page.has_css?('input#comment-save-btn'), "No new comment by gclarke."
      assert page.has_button?("Save"), "No new comment Save button, so no new comment available for edit."
    end
  end
end
