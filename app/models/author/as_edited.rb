...
class Author::AsEdited < Author::AsTypeahead
  ...
  def self.create(params, typeahead_params,username)
    ...
    if author.save_with_username(username)
      ...
      raise "#{author.errors.full_messages.first}"
    end
  rescue => e
    logger.error("Author::AsEdited:rescuing: #{e.to_s}")
  ...
  def update_if_changed(params,typeahead_params,username)
    ...
    if changed?
      ...
      logger.debug("Author::AsEdited #{self.inspect}")
    ...
    logger.error("Author::AsEdited with params: #{e.to_s}")
    raise 
  end
  ...
  def resolve_typeahead_params(params)
    ...
    self.duplicate_of_id = Author::AsEdited.duplicate_of_from_typeahead(params[:duplicate_of_id],params[:duplicate_of_typeahead],id) if params.has_key?(:duplicate_of_id)
  rescue => e
    logger.error("Author::AsEdited:resolve_typeahead_params:found error: #{e.to_s}")
    raise
  end
