...
class Audit::DefinedQuery::WhereClause::Predicate
  ...
  def initialize(sql,field = nil,value = nil,record_type)
    debug("Start field: #{field}; value: #{value}; record_type: #{record_type};")
    @sql = sql
  ...
  def build_predicate(field,value)
    ...
    if field.blank? && value.blank?
      ...
      elsif RECORD_TYPE_ASSERTIONS.has_key?("#{@record_type}-#{canonical_field}")
      ...
      elsif WHERE_VALUE_HASH_2_VALUES.has_key?(canonical_field)
  ...
  def canon_field(field)
    if WHERE_INTEGER_VALUE_HASH.has_key?(field)
      ...
      field
    elsif RECORD_TYPE_ASSERTIONS.has_key?("#{@record_type}-#{field}")
      field
    elsif WHERE_VALUE_HASH.has_key?(field)
      field
    elsif WHERE_VALUE_HASH_2_VALUES.has_key?(field)
      field
      ...
      raise "Cannot audit authors for: #{field}" unless CANONICAL_FIELD_NAMES.has_key?(field)
    end
  end
