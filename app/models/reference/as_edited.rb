...
class Reference::AsEdited < Reference::AsTypeahead
  ...
  def self.create(params, typeahead_params,username)
    ...
    if reference.save_with_username(username)
      ...
      raise "#{reference.errors.full_messages.first}"
    end
    reference
  rescue => e
    logger.error("Reference::AsEdited:rescuing: #{e.to_s}")
  ...
  def resolve_typeahead_params(params)
    self.author_id = Reference::AsEdited.author_from_typeahead(params['author_id'],params['author_typeahead']) if params.has_key?('author_id')
    self.parent_id = Reference::AsEdited.parent_from_typeahead(params["parent_id"],params["parent_typeahead"]) if params.has_key?('parent_id')
    self.duplicate_of_id = Reference::AsEdited.duplicate_of_from_typeahead(params["duplicate_of_id"],params["duplicate_of_typeahead"]) if params.has_key?('duplicate_of_id')
  rescue => e
    logger.error("Reference::AsEdited:resolved_typeahead_params: rescuing exception: #{e.to_s}")
    raise
  end 
  ...
  def self.author_from_typeahead(id_string,text)
    ...
    case self.resolve_id_and_text(id_string,name_text)
      when :no_id_or_text
        value = ''
  ...
  def self.duplicate_of_from_typeahead(id_string,text)
    ...
    case self.resolve_id_and_text(id_string,citation_text)
      when :no_id_or_text
        value = ''
      when :id_only # assume delete
        value = ''
