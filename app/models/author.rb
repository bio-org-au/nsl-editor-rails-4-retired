...
class Author < ActiveRecord::Base
  ...
  scope :lower_name_equals, ->(string) { where("lower(name) = ? ", string.downcase) }
  ...
  has_many :ex_base_names, foreign_key: "ex_base_author_id", class_name: "Name"
  has_many :sanctioned_names, foreign_key: "sanctioning_author_id", class_name: "Name"

  belongs_to :duplicate_of, class_name: "Author", foreign_key: "duplicate_of_id"
  has_many :duplicates, -> { order("name") }, class_name: "Author", foreign_key: "duplicate_of_id", dependent: :restrict_with_error
  has_many :comments

  validates :name, presence: { if: "abbrev.blank?", message: "can't be blank if abbrev is blank." }
  validates :abbrev, presence: { if: "name.blank?", message: "can't be blank if name is blank." }
  validates_exclusion_of :duplicate_of_id, in: ->(author) { [author.id] }, allow_blank: true, message: "and master cannot be the same record"
