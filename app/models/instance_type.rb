...
class InstanceType < ActiveRecord::Base
  ...
  def self.info_or_help_links
    self.all.order(:name).collect do |instance_type|
      %Q(<li><a tabindex="-1" href="#" class="append-to-query-field" data-value="#{instance_type.name}">#{instance_type.name}</a></li>)
    end
  end

  # For new records: just the standard set.
  def self.standalone_options
    self.where('standalone').where.not('deprecated').sort{|x,y| x.name <=> y.name}.collect {|i| [ i.name, i.id ]}
  end

  # For new records: just the standard set.
  def self.synonym_options
    self.where('citing').where.not('deprecated').sort{|x,y| x.name <=> y.name}.collect {|i| [ i.name, i.id ]}
  end

  # For new records: just the standard set.
  def self.unpublished_citation_options
    self.where('unsourced').where.not('deprecated').sort{|x,y| x.name <=> y.name}.collect {|i| [ i.name, i.id ]}
  end
