...
class Search::OnOrchids::FieldRule
  RULES = {
    ...
    "some-name-match:"    => { where_clause: "exists (select null from name where (taxon = name.simple_name or alt_taxon_for_matching = name.simple_name))" ,
                                      order: "seq"},
    "many-name-match-raw:"    => { where_clause: "1 <  (select count(*) from name where (taxon = name.simple_name or alt_taxon_for_matching = name.simple_name) and exists (select null from name_type nt where name.name_type_id = nt.id and nt.scientific))" ,
                                      order: "seq"},
    "many-name-match:"    => { where_clause: "1 <  (select count(*) from name where (taxon = name.simple_name or alt_taxon_for_matching = name.simple_name) and not exists (select null from orchids_names orn where orchids.id = orn.orchid_id) and exists (select null from name_type nt where name.name_type_id = nt.id and nt.scientific))" ,
                                      order: "seq"},
