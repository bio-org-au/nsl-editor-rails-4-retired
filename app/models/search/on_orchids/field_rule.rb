...
class Search::OnOrchids::FieldRule
  RULES = {
    ...
    "no-name-match:"      => { where_clause: "not exists (select null from name where taxon = name.simple_name and exists (select null from name_type nt where name.name_type_id = nt.id and nt.scientific))" },
    "some-name-match:"    => { where_clause: "exists (select null from name where taxon = name.simple_name)" },
    "many-name-match:"    => { where_clause: "1 <  (select count(*) from name where taxon = name.simple_name and exists (select null from name_type nt where name.name_type_id = nt.id and nt.scientific))" },
    "one-name-match:"     => { where_clause: "1 =  (select count(*) from name where taxon = name.simple_name and exists (select null from name_Type nt where name.name_type_id = nt.id and nt.scientific))" },
    "name-match-no-primary:"     => { where_clause: "1 =  (select count(*) from name where taxon = name.simple_name and exists (select null from name_Type nt where name.name_type_id = nt.id and nt.scientific and not exists (select null from instance join instance_type on instance.instance_type_id = instance_type.id where instance_type.primary_instance and name.id = instance.name_id)))" },
    "name-match-eq:"      => { where_clause: "? =  (select count(*) from name where taxon = name.simple_name and exists (select null from name_Type nt where name.name_type_id = nt.id and nt.scientific))" },
    "name-match-gt:"      => { where_clause: "? <  (select count(*) from name where taxon = name.simple_name and exists (select null from name_Type nt where name.name_type_id = nt.id and nt.scientific))" },
