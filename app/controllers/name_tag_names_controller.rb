...
class NameTagNamesController < ApplicationController
  ...
  def create_old
    ...
    if @name_tag_name.save_new_record_with_username(current_user.username)
      ...
      raise "Failed to set tag"
    end
  rescue => e
    logger.error("Name Tag Name create failed.")
    logger.error(e.to_s)
    @message = "Could not attach that tag because #{e.to_s}"
  ...
  def create
    sql = "INSERT INTO name_tag_name (name_id, tag_id, created_at, created_by,
    ...
    logger.error("Name Tag Name create failed: #{e.to_s}")
    @message = "Could not attach that tag because #{e.to_s}"
    render :create_failed, format: :js
  end
