...
module ApplicationHelper
  ...
  def show_field_as_link(text, url, title, label, label_is = :description, _content_is = :text)
    content_tag(:section,
                content_tag(:article,
  ...
  def created_by_whom_and_when(record)
    %(Created <span class="purple">#{time_ago_in_words(record.created_at)}&nbsp;ago</span> by #{record.created_by.downcase} #{formatted_timestamp(record.created_at)})
  end
  ...
  def updated_by_whom_and_when(record)
    ...
    if (record.created_at.to_f / 10).to_i == (record.updated_at.to_f / 10).to_i
      ...
      %(Last updated <span class="purple">#{time_ago_in_words(record.updated_at)}&nbsp;ago</span> by #{record.updated_by.downcase} #{formatted_timestamp(record.updated_at)})
    end
  end
end
