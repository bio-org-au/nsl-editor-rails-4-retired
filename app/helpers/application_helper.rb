...
module ApplicationHelper
  ...
  def entity_icon(entity, height = 10, width = 10)
    color_for = { "name" => "rgba(102,183,39,1.0)",
                  "author" => "rgba(52,114,218,1.0)",
                  "reference" => "rgba(245,172,0,1.0)",
                  "instance" => "rgba(197,151,203,1.0)" }
    content_tag(:svg,
                tag(:polygon,
                    class: "svgpolygon",
  ...
  def help_topic_marked_up_text(name)
    ...
    if help_topic.present?
      parse_markdown(help_topic.marked_up_text)
    else
      "No help found. Expected: #{name}"
    end
  end
  ...
  def show_ref_parent_link(label, reference, _contents_method, url, title)
    if reference.parent
      content_tag(:section,
                  content_tag(:article,
                              content_tag(:a,
  ...
  def show_children_query_link(label,
                               parent,
                               children_method,
                               url,
                               title,
  ...
  def show_field(label,
                 contents,
                 label_is = :description,
                 content_is = :text,
                 field_class = "width-50-percent",
                 label_class = "")
    content_tag(:section,
                content_tag(:article,
                            treated_content(contents, content_is),
                            class: "field-data inline-block #{field_class}") +
                content_tag(:label,
                            treated_label(label, label_is),
                            class: "field-label inline
                            pull-right #{label_class}"),
                class: "field-data")
  end
  ...
  def editable_text_field(form_field, label, label_is = :description)
    content_tag(:section,
                form_field + content_tag(:label,
                                         treated_label(label, label_is),
                                         class: "inline pull-right") +
                             tag(:div,
                                 class: "field-error-message width-80-percent"),
                class: "editable-text-field block")
  end

  def typeahead_field(form_field, label, label_is = :description)
    label_class = "block-inline field-label
      align-right width-20-percent pull-right"
    content_tag(:section,
                form_field +
                content_tag(:label,
                            treated_label(label, label_is),
