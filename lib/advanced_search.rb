...
module AdvancedSearch
  ...
  def parse_search_for_limit(raw_search_request)
    ...
    return calculated_limit,raw_search_request.gsub(/limit:[^ ][^ ]*/,'')
  end
  ...
  def generic_bindings_to_where(active_record_class,search_terms_array)
      ...
      if pairing.size == 2         
        ...
        when 'cr-a'
          unless pairing[1].blank?  || !pairing[1].gsub(/[0-9]/,'').blank?
            ...
            binds.push(Date.today.to_time - ((pairing[1].to_i - 1) * 86400))
          end
        when 'cr-b'
          unless pairing[1].blank?  || !pairing[1].gsub(/[0-9]/,'').blank?
            where += " and created_at < ? "
  ...
  def generic_bindings_to_order_by(search_terms_array,legal_to_order_by = Hash.new)
    ...
    return order_by_binds,rejected_pairings
  end
  
  def interpret_sort_component(component)
    if component.split.size == 1 then
      # assume ascending
      field_descriptor = component.split.first
      sort_direction = 'asc'
    elsif component.split.size == 2 then
    ...
    return field_descriptor,sort_direction
  end
